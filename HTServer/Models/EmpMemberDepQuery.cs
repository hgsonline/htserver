...
namespace HTServer.Models
{
...

    {
        ...
        public async Task<List<EmpMemberDep>> FindMemberAsync(int id)
        {
            ...
            return await ReadAllAsyncI(await cmd.ExecuteReaderAsync());
            //return result.Count > 0 ? result[0] : null;
        }
            ...
            using (reader)
            {
                while (await reader.ReadAsync())
                {
                    var post = new EmpMemberDep(Db)
                    {
                        ...
                        HasMedicalData = await reader.IsDBNullAsync(22) ? false :  await reader.GetFieldValueAsync<int>(22)  == 0 ? false : true
                    };
                    posts.Add(post);
                }
            }
        ...
        private async Task<List<EmpMemberDep>> ReadAllAsyncI(DbDataReader reader)
        {
            var posts = new List<EmpMemberDep>();
            using (reader)
            {
                while (await reader.ReadAsync())
                {
                    var post = new EmpMemberDep(Db)
                    {
                        ...
                        //UpdatedAt = await reader.GetFieldValueAsync<DateTime>(21)
                        UpdatedAt = await reader.IsDBNullAsync(21) ? DateTime.Now.Date : await reader.GetFieldValueAsync<DateTime>(21),
                        HasMedicalData = await reader.IsDBNullAsync(22) ? false : await reader.GetFieldValueAsync<Int64>(22) == 0 ? false : true
                    };
                    posts.Add(post);
                }
            }
